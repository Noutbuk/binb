...
class SongDataManager extends DataManager {
  ...
  setSongMetadata(trackId, metadata, callback) {
    return this._execute(async () => {
      const key = `song:${trackId}`;
      const result = await this.clientAsync.hset([key, ...Object.entries(metadata).flat()]);
      return result;
    }, callback);
  }
  ...
  deleteRoom(roomName, callback) {
    return this._execute(async () => {
      const key = `room:metadata:${roomName}`;
      return Promise.all([this.clientAsync.del(key), this.clientAsync.del(roomName)]);
    }, callback);
  }
   ...
   * @param {string} roomName - The room name
   * @param {Function} [callback] - Optional callback function
   * @returns {Promise<boolean>|undefined} - True if metadata exists or undefined if callback used
   */
  roomMetadataExists(roomName, callback) {
    return this._execute(async () => {
      const metadataKey = `room:metadata:${roomName}`;
      const result = await this.clientAsync.exists(metadataKey);
      return result === 1;
    }, callback);
  }

  /**
   * Get all room names from metadata keys
   * @param {Function} [callback] - Optional callback function
   * @returns {Promise<Array>|undefined} - Array of room names or undefined if callback used
   */
  getAllRoomNamesFromMetadata(callback) {
    return this._execute(async () => {
      const allKeys = await this.clientAsync.keys('room:metadata:*');
      return allKeys.map(key => key.replace('room:metadata:', ''));
    }, callback);
  }

  /**
   * Set room metadata (without song prefix)
   * @param {string} roomName - The room name
   * @param {Object} metadata - Metadata object
   * @param {Function} [callback] - Optional callback function
   * @returns {Promise<string>|undefined} - Redis response or undefined if callback used
   */
  setRoomMetadata(roomName, metadata, callback) {
    return this._execute(async () => {
      const key = `room:metadata:${roomName}`;
   ...
   * @param {string} roomName - The room name
   * @param {Function} [callback] - Optional callback function
   * @returns {Promise<Object>|undefined} - Metadata object or undefined if callback used
   */
  getRoomMetadata(roomName, callback) {
    return this._execute(async () => {
      const key = `room:metadata:${roomName}`;
      const result = await this.clientAsync.hgetall(key);
      return result;
    }, callback);
  }
   ...
   * @param {string} trackId - The track ID
   * @param {Function} [callback] - Optional callback function
   * @returns {Promise<string|null>|undefined} - Score or null if not found, undefined if callback used
   */
  ...
  /**
   * Delete a song by trackId
   * @param {string} trackId - The trackId to delete
   * @param {Function} [callback] - Optional callback function  
   * @returns {Promise<number>|undefined} - Number of keys deleted or undefined if callback used
   */
  deleteSong(trackId, callback) {
    return this._execute(async () => {
      const key = `song:${trackId}`;
      return await this.clientAsync.del(key);
    }, callback);
  }
  ...
  getKeys(pattern, callback) {
    return this._execute(async () => {
      const result = await this.clientAsync.keys(pattern);
      return result;
    }, callback);
  }
...
class UserDataManager extends DataManager {
  constructor(client) {
    ...
    this.clientAsync = {
      ...
      zscore: promisify(client.zscore).bind(client),
      zrem: promisify(client.zrem).bind(client),
      sort: promisify(client.sort).bind(client)
    };
  }
  ...
  getUserCredentials(username, callback) {
    return this._execute(async () => {
      ...
      return await this.clientAsync.hmget(key, ['salt', 'password']);
    }, callback);
  }
  ...
  getBanKeys(prefix, callback) {
    return this._execute(async () => {
      ...
      return await this.clientAsync.keys(pattern);
    }, callback);
  }

  /**
   * Get ban data
   * @param {string} ip - The ban ip
   * @param {Function} [callback] - Optional callback function
   * @returns {Promise<string>|undefined} - Ban data or undefined if callback used
   */
  getBanData(ip, callback) {
    return this._execute(async () => {
      const key = `ban:${ip}`;
      return await this.clientAsync.get(key);
    }, callback);
  }
  ...
  setUserFields(username, fields, callback) {
    return this._execute(async () => {
      const key = `user:${username}`;
      return await this.clientAsync.hset([key, ...Object.entries(fields).flat()]);
    }, callback);
  }
  ...
  getSortedRange(key, start, stop, callback) {
    return this._execute(async () => {
      return await this.clientAsync.zrange([key, start, stop]);
    }, callback);
  }
   ...
   */
  sortKeys(_key, params, callback) {
    return this._execute(async () => {
      // Redis SORT command with the provided parameters
      // params is an array like ['users', 'by', 'user:*->bestguesstime', 'get', '#', 'get', 'user:*->bestguesstime', 'limit', 0, '30']
      return await this.clientAsync.sort(params);
    }, callback);
  }
        ...
        for (const op of operations) {
          try {
            ...
            if (op.op === 'hset') {
              ...
              result = await self.clientAsync.zadd(op.key, op.score, op.member);
            } else if (op.op === 'sadd') {
