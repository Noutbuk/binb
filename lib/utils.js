...
exports.unban = function (ip, spark, callback) {
  ...
  dataService.users.getUserRole(issuedby, function (err, role) {
    if (err) {
    ...
    if (ip !== 'list') {
      return dataService.users.deleteBan(ip);
    }

    // List all banned players
    dataService.users.getBanKeys(usersPrefix, function (err, replies) {
      if (err) {
      ...
      replies.forEach(function (key) {
        ...
        dataService.users.getBanData(key, function (err, reply) {
          if (err) {
            return console.error(err.message);
          }
