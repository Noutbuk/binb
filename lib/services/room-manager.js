...
const { addRoom, removeRoom, reloadRooms } = require('../rooms');
...
class RoomManager {
  ...
  async getAllRooms() {
    try {
      ...
      const metadataRooms = await dataService.songs.getAllRoomNames();
      const rooms = [];
  ...
  async updateRoom(roomName, metadata, updatedBy) {
    try {
      ...
      if (currentMetadata.active && updatedMetadata.active === 'false') {
        removeRoom(roomName);
      } else if (!currentMetadata.active && updatedMetadata.active === 'true') {
        addRoom(roomName);
      }
