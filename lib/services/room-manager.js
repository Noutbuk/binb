...
class RoomManager {
  ...
  async getAllRooms() {
    try {
      // Get all rooms with metadata (new approach)
      const metadataRooms = await dataService.songs.getAllRoomNamesFromMetadata();
      
      // Get legacy rooms that don't have metadata (fallback for existing rooms)
      const allKeys = await dataService.songs.getAllRoomNames();
      const legacyRoomKeys = allKeys.filter(key =>
        !key.startsWith('song:') &&
      ...
      // Filter out legacy rooms that now have metadata to avoid duplicates
      const legacyRoomsWithoutMetadata = [];
      for (const roomKey of legacyRoomKeys) {
        const hasMetadata = await dataService.songs.roomMetadataExists(roomKey);
        if (!hasMetadata) {
          legacyRoomsWithoutMetadata.push(roomKey);
        }
      }
      
      // Combine metadata rooms with legacy rooms
      const roomKeys = [...metadataRooms, ...legacyRoomsWithoutMetadata];
  ...
  async createRoom(roomName, metadata, createdBy) {
    try {
      // Check if room already exists (check metadata first, fallback to sorted set)
      const metadataExists = await dataService.songs.roomMetadataExists(roomName);
      const sortedSetExists = await dataService.songs.songExists(roomName);
      if (metadataExists || sortedSetExists) {
      ...
      await dataService.songs.addTrackToRoom(roomName, 'placeholder', 0);
  ...
  /**
   * Check if room exists (checks metadata first, fallback to sorted set)
   */
  async roomExists(roomName) {
    try {
      ...
      // Fallback to sorted set for legacy rooms without metadata
      return await dataService.songs.songExists(roomName);
    } catch (error) {
