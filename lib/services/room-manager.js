...
class RoomManager {
  ...
  async getRoomMetadata(roomName) {
    try {
      ...
      const metadata = await dataService.songs.getSongMetadata(metadataKey);
  ...
  async createRoom(roomName, metadata, createdBy) {
    try {
      ...
      const exists = await dataService.songs.songExists(roomName);
      if (exists) {
      ...
      const metadataKey = `room:metadata:${roomName}`;
      await dataService.songs.setSongMetadata(metadataKey, roomMetadata);
  ...
  async updateRoom(roomName, metadata, updatedBy) {
    try {
      ...
      const metadataKey = `room:metadata:${roomName}`;
      const currentMetadata = await dataService.songs.getSongMetadata(metadataKey);
      ...
      await dataService.songs.setSongMetadata(metadataKey, updatedMetadata);
