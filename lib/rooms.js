...
module.exports = async function (options) {
  ...
  // Initialize rooms from Redis metadata only
  ...
  try {
    ...
    activeRooms.forEach(function (roomData) {
      rooms[roomData.name] = new Room(roomData.name);
    });
    
    console.log(`Initialized ${activeRooms.length} rooms from Redis:`, activeRooms.map(r => r.name));
  } catch (error) {
    console.error('Failed to load rooms from Redis:', error);
    console.log('No rooms initialized - only Redis-based rooms with metadata are supported');
  }
};
...
/**
 * Dynamic room management functions
 */
...
module.exports.reloadRooms = async function () {
  const roomManager = require('./services/room-manager');
  
  try {
    // Get all active rooms from database
    const allRooms = await roomManager.getAllRooms();
    const activeRooms = allRooms.filter(room => room.active).map(room => room.name);
    ...
    // Update config
    config.rooms = activeRooms;
    
    return { success: true, activeRooms: activeRooms.length };
  } catch (error) {
...
Room.prototype.initialize = function () {
  ...
  dataService.songs.getRoomTrackCount(this.roomname, function (err, card) {
    if (err) {
      throw err;
    }
...
Room.prototype.onKick = function (who, why, executor, duration, callback) {
  ...
  dataService.users.getUserRole(executor, function (err, role) {
    if (err) {
    ...
    if (room.usersData[who]) {
      ...
      if (duration) {
        dataService.users.banUser(target.address.ip, duration, who);
      }
...
Room.prototype.join = function (spark, nickname) {
  ...
  dataService.users.userExists(nickname, function (err, exists) {
...
Room.prototype.sendLoadTrack = function () {
  const room = this;
  room.status = Room.LOADING;

  if (room.trackscount == 0) {
    setTimeout(room.sendLoadTrack.bind(room), 5000);
    return;
  }

  const index = randInt(room.trackscount);

  dataService.songs.getRoomTrackByIndex(this.roomname, index, function (err, res) {
    if (err) {
      throw err;
    }
    ...
    if (~room.playedtracks.indexOf(id)) {
      if (room.playedtracks.length < room.trackscount) {
        return room.sendLoadTrack();
      }
      // All tracks have been played, reset the list
      room.playedtracks = [];
      return room.sendLoadTrack();
    }
    ...
    dataService.songs.getSongMetadata(
      id,
      ['artistName', 'trackName', 'previewUrl', 'artworkUrl60', 'trackViewUrl'],
